...
export const Movie: React.FC<IMovieProps> = ({
  ...
  const [movie, setMovie]: [
    IMovie,
    React.Dispatch<React.SetStateAction<IMovie>>
  ] = useState({ movieId: 0 });
    ...
    async function getMovie() {
      ...
      if (cachedResult) {
        ...
        let response = await fetch(
          `https://api-summit.aws.cliffordduke.dev/movies/${id}`
        );
      ...
      >
        <Box height={imageOnly ? "small" : "medium"}>
          <Image
            ...
            src={
              movie.posterPath
                ? `//assets-summit.aws.cliffordduke.dev/${movie.posterPath}`
                : missing_artwork
            }
              ...
              <Box>
                {showTitle && (
                  <Heading
                    ...
                    level="5"
                    margin="none"
                    textAlign={imageOnly && showTitle ? "center" : "start"}
                  >
                    {movie.title}
                  </Heading>
                )}
            ...
            >
              ...
              >
                <Text color="dark-5" size="xsmall">
                  {loaded && `${movie.totalFavorites} people like this movie`}
                </Text>
              </Box>
            </Box>
          </React.Fragment>
