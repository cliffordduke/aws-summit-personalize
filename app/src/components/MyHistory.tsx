...
export const MyHistory = withRouter(({ match, history: routeHistory }) => {
  ...
  useEffect(() => {
    async function execute() {
      ...
      if (historyCacheResult) {
        ...
        let response = await fetch(
          `https://api-summit.aws.cliffordduke.dev/users/${match.params
            .userId || userId}/recommendation/history`
        );
      ...
      const recommendationCacheKey = `recommendation:${match.params.userId ||
        userId}`;
      const recommendationCacheResult = localStorage.getItem(
    ...
    <Box>
      ...
      <Timeline>
        {history.map((record, index) => (
          <Event
            backgroundColor={record.event === "RECORD" ? "#fdc500" : "#ff5252"}
        ...
        {history.length > 0 && (
          <Event
            ...
            label={`Current generation recommendation`}
          >
            <Grid
              align="start"
              margin={{ left: "small", right: "small", bottom: "medium" }}
